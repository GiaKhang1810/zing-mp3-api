...
const { default: client, Client, Lapse, Cookies, createSignature } = require('@khang07/zing-mp3-api');
...
describe('Client.cjs entry', function () {
...
describe('Client input guards', function () {
    ...
    it('search() rejects empty queries', async function () {
        await assert.rejects(client.search('', 'music'), (error) => isLapseWithCode(error, 'ERROR_INVALID_QUERY'));
    });

    it('search() rejects empty types', async function () {
        await assert.rejects(client.search('Skyfall', ''), (error) => isLapseWithCode(error, 'ERROR_INVALID_TYPE'));
    });

    it('search() rejects unsupported types', async function () {
        await assert.rejects(client.search('Skyfall', 'album'), (error) => isLapseWithCode(error, 'ERROR_INVALID_TYPE'));
    });
});
...
describe('Client options', function () {
    it('getJar() returns the current cookie jar', function () {
        const instance = new Client();
        assert.ok(instance.getJar() instanceof Cookies);
    });

    it('setOptions() keeps the current jar by default', function () {
        const instance = new Client();
        const jar = instance.getJar();

        instance.setOptions({
            maxLoad: 256 * 1024,
            maxHighWaterMark: 8 * 1024,
            userAgent: 'test-agent'
        });

        assert.equal(instance.getJar(), jar);
    });

    it('setOptions() can replace the jar', function () {
        const instance = new Client();
        const jar = new Cookies();

        instance.setOptions({ jar });
...
describe('createSignature()', function () {
    it('returns a deterministic sha512 hex signature', function () {
        const signature = createSignature('/api/test', { a: 1, b: 2 }, 'secret');

        assert.equal(signature.length, 128);
        assert.match(signature, /^[a-f0-9]{128}$/);
        assert.equal(signature, createSignature('/api/test', { a: 1, b: 2 }, 'secret'));
        assert.notEqual(signature, createSignature('/api/test', { a: 1, b: 3 }, 'secret'));
    });
});
...
describeIfLive('Live integration', function () {
    ...
    it('search(music) returns items', async function () {
    ...
    it('search(video) returns items', async function () {
        const items = await client.search('sign', 'video');
    ...
    it('search(playlist) returns items', async function () {
        const items = await client.search('sign', 'playlist');
    ...
    it('search(artist) returns items', async function () {
        const items = await client.search('Bray', 'artist');
