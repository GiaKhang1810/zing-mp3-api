...
function validateOptions(options) {
    ...
    if (typeof mergedOptions.maxDownRate !== 'number' ||
        ...
        throw new lapse.Lapse('`maxDownRate` must be a finite number greater than 0.', 'ERROR_INVALID_OPTIONS');
    if (typeof mergedOptions.highWaterMark !== 'number' ||
        !Number.isFinite(mergedOptions.highWaterMark) ||
        mergedOptions.highWaterMark <= 0)
        throw new lapse.Lapse('`highWaterMark` must be a finite number greater than 0.', 'ERROR_INVALID_OPTIONS');
    if (typeof mergedOptions.userAgent !== 'string' ||
        !mergedOptions.userAgent.trim().length)
        throw new lapse.Lapse('`userAgent` must be a non-empty string.', 'ERROR_INVALID_OPTIONS');
    if (!(mergedOptions.jar instanceof cookies.Cookies))
        throw new lapse.Lapse('`jar` must be an instance of Cookies.', 'ERROR_INVALID_OPTIONS');
