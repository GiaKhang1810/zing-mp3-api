...
const music = async (options) => {
    try {
        ...
        const response = await options.instance.get(options.API_MUSIC_PATH, {
            params: {
                id: options.id,
                sig: encrypt.createSignature(options.API_MUSIC_PATH, {
                    ctime: options.ctime,
                    id: options.id,
        ...
        source.once('error', (error) => {
            const lapse$1 = error instanceof Error
                ? error
...
const musicSync = (options, highWaterMark) => {
    ...
    void music(options)
        ...
        const lapse$1 = error instanceof lapse.Lapse
            ? error
            : new lapse.Lapse(error instanceof Error ? error.message : 'An unknown error occurred while fetching', 'ERROR_MUSIC_FETCH', void 0, error);
        copySource.destroy(lapse$1);
    });
    return copySource;
};
