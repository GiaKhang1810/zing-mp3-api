...
export class Cookies {
    ...
    constructor(setCookie, requestURL) {
        ...
        if (typeof setCookie === 'string') {
            if (typeof requestURL !== 'string' || !requestURL.trim().length)
                throw new Lapse('Request URL must be a non-empty string when setCookie is a string.', 'ERROR_INVALID_URL');
        ...
        if (Array.isArray(setCookie)) {
            ...
            if (isStringArray) {
                if (typeof requestURL !== 'string' || !requestURL.trim().length)
                    throw new Lapse('Request URL must be a non-empty string when setCookie is a string array.', 'ERROR_INVALID_URL');
            ...
            throw new Lapse('Input array must contain only strings or only cookie records.', 'ERROR_INVALID_COOKIE_INPUT');
        }
        if (isCookieRecord(setCookie)) {
            this.fromJSON([setCookie]);
            return;
        }
        throw new Lapse('Invalid cookie input.', 'ERROR_INVALID_COOKIE_INPUT');
    }
    setCookie(setCookie, requestURL) {
        try {
            if (typeof setCookie !== 'string' || !setCookie.trim().length)
                throw new Lapse('Set-Cookie must be a non-empty string.', 'ERROR_INVALID_SET_COOKIE');
            if (typeof requestURL !== 'string' || !requestURL.trim().length)
                throw new Lapse('Request URL must be a non-empty string.', 'ERROR_INVALID_URL');
            ...
            throw new Lapse(message, 'ERROR_SET_COOKIE', undefined, error);
        }
    }
    setCookies(setCookies, requestURL) {
        try {
            ...
            throw new Lapse(message, 'ERROR_SET_COOKIES', undefined, error);
        }
    }
    getCookies(requestURL) {
        try {
            ...
            throw new Lapse(message, 'ERROR_GET_COOKIES', undefined, error);
        }
    }
    getCookieHeader(requestURL) {
        try {
            ...
            throw new Lapse(message, 'ERROR_GET_COOKIE_HEADER', undefined, error);
        }
    }
    applyToHeaders(requestURL, headers = {}) {
        try {
            ...
            throw new Lapse(message, 'ERROR_APPLY_COOKIE_HEADERS', undefined, error);
        }
    }
    deleteCookie(domain, path, name) {
        try {
            if (typeof domain !== 'string' || !domain.trim().length)
                throw new Lapse('Cookie domain must be a non-empty string.', 'ERROR_INVALID_COOKIE_DOMAIN');
            if (typeof path !== 'string' || !path.trim().length)
                throw new Lapse('Cookie path must be a non-empty string.', 'ERROR_INVALID_COOKIE_PATH');
            if (typeof name !== 'string' || !name.trim().length)
                throw new Lapse('Cookie name must be a non-empty string.', 'ERROR_INVALID_COOKIE_NAME');
            ...
            throw new Lapse(message, 'ERROR_DELETE_COOKIE', undefined, error);
        }
    }
    cleanup() {
        try {
            ...
            throw new Lapse(message, 'ERROR_COOKIE_CLEANUP', undefined, error);
        }
    }
    toJSON() {
        try {
            ...
            throw new Lapse(message, 'ERROR_COOKIE_TO_JSON', undefined, error);
        }
    }
    fromJSON(cookies) {
        try {
            if (!Array.isArray(cookies))
                throw new Lapse('Cookies must be an array.', 'ERROR_INVALID_COOKIES_JSON');
            this.store.clear();
            for (const cookie of cookies) {
                if (!cookie ||
                    ...
                    throw new Lapse('Invalid cookie record in JSON input.', 'ERROR_INVALID_COOKIE_RECORD');
                if (isExpired(cookie, Date.now()))
                    continue;
                let byPath = this.store.get(cookie.domain);
                if (!byPath) {
            ...
            throw new Lapse(message, 'ERROR_COOKIE_FROM_JSON', undefined, error);
        }
    }
}
